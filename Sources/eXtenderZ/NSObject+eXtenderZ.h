 ...
 * Can be called more than once on an extender instance
 *  if the extender is added on more than one object,
 ...
 * “protocol” must conform to protocol HPNExtender, the registered class must respond to protocol HPNHelptender. */
 ...
 * It returns YES if the extender was added, NO if it was not
 *  (the extender refused to be added or it was already added to the object).
 *
 * The protocols to which the extender responds to will determine which helptenders will be added to the object.
 *
 * IMPORTANT NOTE:
 * All added extenders are removed from the object just the moment _before_ dealloc is called.
 * Do *not* try to access extensions properties or call extensions methods in dealloc!
 * You can however _prepare_ the deallocation of said object by overriding hpn_prepareDeallocationOfExtendedObject.
 ...
 * If you want to be called when an extender is added to an object, see hpn_prepareForExtender: */
- (BOOL)hpn_addExtender:(NSObject <HPNExtender> *)extender;
/* Removes given extender.
...
/* Removes the extenders in the given array, returns the number of extenders actually removed. */
...
/* Called in hpn_addExtender:, _after_ the helptender(s) have been added to the object.
 ...
 * You must check whether calling super (don’t forget to use HELPTENDER_CALL_SUPER_* methods to call super) returns YES or NO.
 * If it returns NO, you must return NO right away. */
- (BOOL)hpn_prepareForExtender:(NSObject <HPNExtender> *)extender;

/* Remove the given extender at the given index.
