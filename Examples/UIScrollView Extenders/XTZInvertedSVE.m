...
/* Note: Logs are commented for performance reasons. (Not sure it's 100% needed, but it feels safer.) */
@implementation XTZInvertedSVE
...
//		NSLog(@"Setting content offset to %@ (was %@).", NSStringFromCGPoint(contentOffset), NSStringFromCGPoint(scrollView.contentOffset));
...
//	NSLog(@"Will change content size from %@ to %@.", NSStringFromCGSize(originalSize), NSStringFromCGSize(newSize));
...
//	NSLog(@"Did change content size from %@ to %@.", NSStringFromCGSize(originalSize), NSStringFromCGSize(newSize));
...
//	NSLog(@"Will change content inset from %@ to %@.", NSStringFromUIEdgeInsets(originalInset), NSStringFromUIEdgeInsets(newInset));
