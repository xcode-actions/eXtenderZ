...
#import "XTZCustomCancelableTouchesSVE.h"
...
@interface XTZCustomCancelableTouchesSVESettings : NSObject
...
@implementation XTZCustomCancelableTouchesSVESettings
...
{
	XTZCustomCancelableTouchesSVESettings *ret = [XTZCustomCancelableTouchesSVESettings new];
...
+ (instancetype)settingsFromExtender:(XTZCustomCancelableTouchesSVE *)extender
{
...
/* Note:
 * The implementation of this extender is weird (exchange of implementations of two methods which do not even belong to the same class…).
 * At the time of writing of this extender, the concept of helptender did not exist,
 *  and it was difficult to extend a method which was not meant to be extended by the original extender implementation.
 * Now, the correct thing to do would either to extend the scroll view helptender so it allows extending redirectedTouchesShouldCancelInContentView:,
 *  or create a new scroll view helptender dedicated to this extender (the choice should consider performance issues). */
@implementation XTZCustomCancelableTouchesSVE
...
- (BOOL)prepareObjectForExtender:(UIScrollView *)scrollView
{
	...
	dispatch_once(&onceToken, ^{
		method_exchangeImplementations(class_getInstanceMethod([UIScrollView class], @selector(touchesShouldCancelInContentView:)),
												 class_getInstanceMethod([XTZCustomCancelableTouchesSVE class], @selector(redirectedTouchesShouldCancelInContentView:)));
	});
...
- (void)registerScrollView:(UIScrollView *)scrollView
{
	...
	CFDictionarySetValue(specialClassesInfos, scrollView, [XTZCustomCancelableTouchesSVESettings settingsFromExtender:self]);
}
...
- (BOOL)redirectedTouchesShouldCancelInContentView:(UIView *)view
{
	...
	XTZCustomCancelableTouchesSVESettings *i = (XTZCustomCancelableTouchesSVESettings *)CFDictionaryGetValue(specialClassesInfos, self /* self is here the UIScrollView, not the extender */);
	NSAssert(i == nil || [i isKindOfClass:XTZCustomCancelableTouchesSVESettings.class],
				@"INTERNAL ERROR: Got invalid infos %@, not of class XTZCustomCancelableTouchesSVESettings.", i);
...
callSuper:;
	BOOL (*imp)(id, SEL, UIView *) = (BOOL (*)(id, SEL, UIView *))class_getMethodImplementation([XTZCustomCancelableTouchesSVE class], @selector(redirectedTouchesShouldCancelInContentView:));
