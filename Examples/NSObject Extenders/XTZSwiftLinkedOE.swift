...
/** 
 Abstract implementation of an object extender.
 The class guarantees the extender will be linked to one and only one object, of class `T`.
 ...
 - Important:
 When calling super for the `prepareObjectForExtender(object:)` method,
  the subclass must know for certain it won't refuse adding the given view controller.
 ...
 ``` */
class XTZSwiftLinkedOE<T : NSObject> : NSObject, XTZExtender {
	...
	func prepareObjectForRemoval(ofExtender object: NSObject) {
		...
		assert(linkedObject == nil || object === linkedObject, "Asked to prepare for removal of extender in linked OE, with object (\(String(describing: object))) != _linkedObject (\(String(describing: linkedObject))).")
		linkedObject = nil;
	}
