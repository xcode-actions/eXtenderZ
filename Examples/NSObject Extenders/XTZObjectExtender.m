...
@implementation XTZObjectHelptender

+ (void)load
{
	[self xtz_registerClass:self asHelptenderForProtocol:@protocol(XTZObjectExtender)];
}
...
{
	if (((BOOL (*)(id, SEL, SEL))XTZ_HELPTENDER_CALL_SUPER(XTZObjectHelptender, aSelector)))
		return YES;
	
	NSArray *extendersConformingToProtocol = [self xtz_extendersConformingToProtocol:@protocol(XTZObjectExtender)];
	if (extendersConformingToProtocol.count == 0) return NO;
	
	aSelector = transformedSelectorFrom(aSelector);
	if (aSelector == NULL) return NO;
	
	for (id <XTZObjectExtender> extender in extendersConformingToProtocol)
...
{
	if (((BOOL (*)(id, SEL, SEL))XTZ_HELPTENDER_CALL_SUPER_WITH_SEL_NAME(XTZObjectHelptender, respondsToSelector:, aSelector)))
		return ((NSMethodSignature *(*)(id, SEL, SEL))XTZ_HELPTENDER_CALL_SUPER(XTZObjectHelptender, aSelector));
	...
	for (NSObject <XTZObjectExtender> *extender in extendersConformingToProtocol) {
		if ([extender respondsToSelector:aSelector]) {
...
{
	/* We could forward the invocation to all the extenders that respond to it (doc says so),
	 *  but I think it's clearer to only forward to the first one that responds to the invocation's selector
	 *  (how would we "merge" return values for methods returning something, etc.). */
...
//		NSLog(@"Frame length (%"NSUINT_FMT") is greater than static buffer length (%"NSUINT_FMT"). Allocating a dynamic buffer.", anInvocation.methodSignature.frameLength, (NSUInteger)DEFAULT_BUFFER_LENGTH);
	...
	for (id <XTZObjectExtender> extender in [self xtz_extendersConformingToProtocol:@protocol(XTZObjectExtender)]) {
	...
	((void (*)(id, SEL, NSInvocation *))XTZ_HELPTENDER_CALL_SUPER(XTZObjectHelptender, anInvocation));
