...
/* WARNING:
 * When using an extended table view,
 *  if you plan to use extenders that transform the section/row indexes,
 *  you should always use the `xtz_transformedIndexPath:` and `xtz_transformedSectionIndex:` methods
 *  to get the actual row/section index to or from a datasource row/section index
 *  (as the transformation is a bijection, transforming from or to actual row/section is the same operation). */

@protocol XTZTableViewExtender <XTZExtender>
@optional
 ...
 * If implementation returns YES, the extender can use the viewInTableHeaderForExtender: (resp. viewInTableFooterForExtender:) method
 *  to get their views in the header (resp. footer) view of the table view.
 * The methods can be used even in the prepareTableViewForExtender: method implementation of the extender.
 ...
 * When the extender is removed from a table view, its associated subview in the header or footer view will be removed.
 * However, the size of the header/footer view won’t be modified automatically, you have to do it in the prepareObjectForRemovalOfExtender: method. */
 ...
 * The transformedSectionIndexFrom:…, transformedRowIndexFrom:… must of course be implemented when implementing these methods.
 * It is also an error to implement the first of these method but not the second one or vice-versa. */
...
/* Gives the extender an opportunity to modify the order of the sections from the one the data source gives,
 *  or to implement the division introduced by transformedNumberOfSectionsFrom:… row is _not_ transformed before this method is called.
 * It can be -1 if there are no row information known at the time of calling.
...
/* Gives the extender an opportunity to modify the order of the rows from the one the data source gives.
...
/* Allows to customize how the cell looks (for instance you can set a custom background).
 * In the implementation of this method, you can (should) use the isTransformedByExtender:/setTransformed:byExtender: methods of the cell.
 * These methods are created so you can avoid applying the transformation more than once on the same cell (cells are reused). */
...
/* When transforming a cell (see above), you can (understand should) use these methods to avoid transforming the cell more than once. */
 ...
 * As some extenders can change the available width for a cell,
 *  don’t use tableview.bounds.size.width to get the available width for a cell when using an extended table view.
...
/* Asking the view for an extender which is not registered for the given table view will throw an exception. */
...
/* This method sets the data source and the delegate of the table view.
 * It is recommended to use this method instead of
 *    tableView.delegate = …; tableView.dataSource = …;
 *  for performance (for extended table views) and stability (probably only for extended table views too) reasons.
 *
 * In some rare cases, setting the data source before the delegate might make the table view crash
 *  (probably for extended table views only, but untested on non-extended table views).
 *
 * See XTZProfileDisplayerALVCE.swift, prepareObjectForRemovalOfExtender(_:) for more information. */
...
@interface XTZTableViewDelegateDataSourceForHelptender : XTZScrollViewDelegateForHelptender <UITableViewDataSource, UITableViewDelegate> {
@private
...
@interface XTZTableViewHelptender : UITableView <XTZHelptender>
