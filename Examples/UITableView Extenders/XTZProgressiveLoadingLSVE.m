...
#import "XTZProgressiveLoadingLSVE.h"
...
@interface XTZProgressiveLoadingLSVE ()
...
@implementation XTZProgressiveLoadingLSVE
...
- (id)init
{
	if ((self = [super init]) != nil) {
		self.viewLoadingMore = [XTZLoadingMoreView loadingMoreView];
		self.topMargin = 0.;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
	if (delegateRespondsToDidReachEnd) {
		...
		if ((!_inverted && scrollView.contentOffset.y + margin > MAX(scrollView.contentSize.height - scrollView.frame.size.height, margin)) ||
			...
			[(id <XTZProgressiveLoadingScrollViewDelegate>)scrollView.delegate progressiveLoadingScrollViewDidReachEnd:scrollView];
		}
	}
}
...
- (void)object:(UIScrollView *)scrollView setHasMore:(BOOL)flag
{
	...
	if (hasMore) {
		...
		if (_inverted) {
			/* Let's remove the inset added by the "Loading…" message:
			 *  We hide the “You’ve reached the end” message if inverted. */
			if (insetting) {
				UIEdgeInsets inset = scrollView.contentInset;
				inset.top -= topInset; topInset = 0.;
				scrollView.contentInset = inset;
...
- (void)setViewLoadingMore:(UIView <XTZLoadingMoreView> *)v
{
