...
func writePackageFile(version: String?, checksums: [String: String?]) throws {
	...
	packageString.append(types.map{ type in
		...
		if let checksum = checksum {
			return #"\#t\#t.binaryTarget(name: "eXtenderZ-\#(type)", url: "https://github.com/xcode-actions/eXtenderZ/releases/download/\#(version!)/eXtenderZ-\#(type).xcframework.zip", checksum: "\#(checksum)")"#
		} else {
